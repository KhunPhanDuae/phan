...
function setProfileFieldsEditable(editable) {
    ...
    saveProfileButton.classList.toggle('hidden', !editable); // Hide save button if not editable
}
...
async function loadUserProfile(uid) {
    try {
        ...
        if (snapshot.exists()) {
            const userData = snapshot.val();
            profileIdDisplay.value = uid; // User ID is always read-only
            ...
            profileError.textContent = "Profile data not found.";
        }
    } catch (error) {
